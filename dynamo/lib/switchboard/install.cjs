...
const { DYNAMO_DIR, output, error, loadEnv, safeReadFile } = require(path.join(__dirname, '..', 'core.cjs'));
...
function generateConfig(envPath, outDir) {
  ...
  const config = {
    version: (safeReadFile(path.join(REPO_DIR, 'VERSION')) || '0.1.0').trim(),
    graphiti: {
...
async function run(args = [], pretty = false) {
  ...
  // Step 1: Copy files
  try {
    const fileCount = copyTree(REPO_DIR, LIVE_DIR, INSTALL_EXCLUDES);
